...
function run_tables_tests()
	@testset "Tables" begin
		...
		basic_table = SCP.create_table(col_args...)
		df = DataFrame(col_args...)

		csv_filename = tempname()*".csv"
		CSV.write(csv_filename, df)
		csv_table = SCP.load_csv(csv_filename)


		basic_table_p = SCP.create_table(col_args_p...)
		df_p = DataFrame(col_args_p...)

		csv_filename_p = tempname()*".csv"
		CSV.write(csv_filename_p, df_p)
		csv_table_p = SCP.load_csv(csv_filename_p)
		...
		@testset "$name" for (name,table) in (("DataFrame",df), ("create_table",basic_table), ("CSV",csv_table))
			...
			@test fetch!(SCP.get_colnames(table)) == ["id", "x", "y"]
			@test fetch!(SCP.get_id_colname(table)) == "id"
			@test fetch!(SCP.id_column(table)) == select(df, "id"; copycols=false)
			@test fetch!(SCP.table_nrow(table)) == n
			@test fetch!(SCP.table_ncol(table)) == 3

			let cols = ["y", "id"]
				@test isequal(fetch!(SCP.get_columns(table, cols...)), select(df, cols))
			end
			let cols = ["y", "not_a_column_name", "id"]
				@test_throws ReproducibleJobs.ProcessingException fetch!(SCP.get_columns(table, cols...))
			end

			@test isequal(fetch!(SCP.column_data(table, "id")), df.id)
			@test isequal(fetch!(SCP.column_data(table, "x")), df.x)
			@test isequal(fetch!(SCP.column_data(table, "y")), df.y)
			@test_throws ReproducibleJobs.ProcessingException fetch!(SCP.column_data(table, "not_a_column_name"))


			annot = SCP.annotation(table, "y")
			...
			@test fetch!(SCP.get_id_colname(annot)) == "id"
			@test fetch!(SCP.get_value_colname(annot)) == "y"
			@test_throws ReproducibleJobs.ProcessingException fetch!(SCP.get_value_colname(table))

			@test fetch!(SCP.id_column(annot)) == select(df, "id")
			@test fetch!(SCP.value_column(annot)) == select(df, "y")
			@test_throws ReproducibleJobs.ProcessingException fetch!(SCP.value_column(table))

			@test fetch!(SCP.id_column_data(annot)) == df[!,"id"]
			@test fetch!(SCP.value_column_data(annot)) == df[!,"y"]
			@test_throws ReproducibleJobs.ProcessingException fetch!(SCP.value_column_data(table))


			let u_data = sqrt.(1:n)
				df2 = insertcols(df, "u"=>u_data)
				table2 = SCP.add_column(table, "u", u_data)
				@test isequal(fetch!(table2), df2)
				@test fetch!(SCP.table_ncol(table2)) == 4
			end
			...
			@testset "table_hcat with $name_hc" for (name_hc,table_hc) in (("DataFrame",df_hc), ("create_table",basic_table_hc), ("CSV",csv_table_hc))
				table2 = SCP.table_hcat(table, table_hc)
			...
			@testset "Leftjoin with $name" for (name,table_right) in (("DataFrame",df_right), ("create_table",basic_table_right), ("CSV",csv_table_right))
				table2 = SCP.table_leftjoin(table, table_right)
				@test isequal(fetch!(table2), leftjoin(df, df_right; on=:id, order=:left))
				@test fetch!(SCP.table_ncol(table2)) == 5
			end
		end
		...
		@testset "ReplaceFilename" begin
			csv_table_p = SCP.project(csv_table, TimestampedFilePath(csv_filename)=>TimestampedFilePath(csv_filename_right))
		...
		@testset "transform_annotation $name" for (name,table,table_p) in (("DataFrame",df,df_p), ("create_table",basic_table,basic_table_p), ("CSV",csv_table,csv_table_p))
			annot_x = SCP.annotation(table, "x")  # id + x (numeric)
			x_vals = Float64.(df.x)

			@testset "simple transform" begin
				ta = SCP.transform_annotation(sqrt, annot_x)
				@test isequal(fetch!(ta), DataFrame("id"=>df.id, "x"=>sqrt.(x_vals)))
			end

			@testset "new_name" begin
				ta = SCP.transform_annotation(sqrt, annot_x; new_name="sqrtx")
			...
			@testset "prefetched scalar inside Base.Fix2" begin
				...
				# projections
				# ta_p = SCP.project(ta, table=>table_p) # Hmm. This doesn't work. Because `table` isn't kept as a unit during projection.
