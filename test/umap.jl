...
function run_umap_tests()
	@testset "UMAP" begin
		...
		umap_job = SCP.umap(pca_job; ndim=2, seed=9876)
		...
		umap_proj_job = SCP.project(umap_job, counts_job=>counts_sub_job)
		umap_proj_dm = fetch!(umap_proj_job)

		Y = fetch!(SCP.project(SCP.get_matrix(pca_job), counts_job=>counts_sub_job))
