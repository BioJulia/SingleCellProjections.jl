	...
	@testset "Sum Squared" begin
		...
		@testset "projections" begin
			...
			@testset "$col projections" for (f, g, col) in (
				...
				proj_yes_job = Jobs.project(f(normalized_job; assume_centered=true, project=:yes), normalized_job => normalized_sub_job)
		...
		@testset "normalize_matrix annotate_variance" begin
			...
			@testset "$col" for (col_kwarg, g, col) in (
				...
				job = Jobs.normalize_matrix(Jobs.sctransform(counts_job); col_kwarg=>col)
				data = fetch!(job)
				@test data isa DataMatrix
				@test col in names(data.var)
				@test data.var[!, col] ≈ expected

				job2 = Jobs.normalize_matrix(Jobs.sctransform(counts_job); col_kwarg=>"my_$col")
				data2 = fetch!(job2)
			...
			@testset "annotate_* shortcut" begin
				job = Jobs.normalize_matrix(Jobs.sctransform(counts_job);
				...
				@test data.var[!, "variance"] ≈ fetch!(Jobs.variance(normalized_job; assume_centered=true))[!, "variance"]
				@test data.var[!, "std"] ≈ fetch!(Jobs.std(normalized_job; assume_centered=true))[!, "std"]
				@test data.var[!, "relative_std"] ≈ fetch!(Jobs.relative_std(normalized_job; assume_centered=true))[!, "relative_std"]
			end

			@testset "variance annotation preserved under projection" begin
				job_with_var = Jobs.normalize_matrix(Jobs.sctransform(counts_job); annotate_variance=true, annotate_std=true, annotate_relative_std=true)
				proj_job = Jobs.project(job_with_var, counts_job => counts_sub_job)
			...
			@testset "center=false error" begin
				@test_throws "requires center=true" fetch!(Jobs.normalize_matrix(Jobs.sctransform(counts_job); center=false, annotate_variance=true))
			end
		end
