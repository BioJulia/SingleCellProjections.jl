...
function run_filter_tests()
	@testset "Filtering" begin
		...
		reduced_job = SCP.pca(normalized_job; nsv=10, seed=1234)
		...
		# @testset "filter $name" for (name,data_job) in (("counts",counts_job),)
			...
			f_job = SCP.filter_matrix(:, :, data_job)
			let f = fetch!(f_job)
			...
			f_job = SCP.filter_var(1:2:P, data_job)
			let f = fetch!(f_job)
			...
			f_job = SCP.filter_obs(1:2:N, data_job)
			let f = fetch!(f_job)
			...
			f_job = SCP.filter_matrix(1:3:P, 1:10:N, data_job)
			let f = fetch!(f_job)
			...
			f_job = SCP.filter_obs("group"=>==("A"), data_job)
			@test forward!(SCP.get_var(f_job)) == var_spec_forwarded
			let f = fetch!(f_job)
			...
			# f_job = SCP.filter_obs(["group","value"]=>(g,v)->g=="A" && v<1.0, data_job)
			# let f = fetch!(f_job)
			...
			f_job = SCP.filter_matrix(1:3:P, "group"=>==("A"), data_job)
			let f = fetch!(f_job)
			...
			f_job = SCP.filter_var("name"=>>("D"), data_job)
			let f = fetch!(f_job)
			...
			f_job = SCP.filter_matrix("name"=>>("D"), "group"=>==("A"), data_job)
			let f = fetch!(f_job)
		...
		@testset "getindex collapsing" begin
			...
			j21 = SCP.filter_obs(f1, SCP.filter_obs(f2, counts_job))
			j12 = SCP.filter_obs(f2, SCP.filter_obs(f1, counts_job))

			@test isequal(forward!(SCP.get_matrix(j21)), forward!(SCP.get_matrix(j12))) # collapsing of matrix indexing
			@test isequal(forward!(SCP.get_var(j21)), forward!(SCP.get_var(j12))) # easier, vars are not filtered
			@test isequal(forward!(SCP.get_obs(j21)), forward!(SCP.get_obs(j12))) # collapsing of table indexing
			...
			f3 = "value"=><(1.8)
			j321 = SCP.filter_obs(f1, SCP.filter_obs(f2, SCP.filter_obs(f3, counts_job)))
			j123 = SCP.filter_obs(f3, SCP.filter_obs(f2, SCP.filter_obs(f1, counts_job)))

			@test isequal(forward!(SCP.get_matrix(j321)), forward!(SCP.get_matrix(j123))) # collapsing of matrix indexing
			@test isequal(forward!(SCP.get_var(j321)), forward!(SCP.get_var(j123))) # easier, vars are not filtered
			@test isequal(forward!(SCP.get_obs(j321)), forward!(SCP.get_obs(j123))) # collapsing of table indexing

			# @show forward!(SCP.get_obs(j321))
			# @show forward!(SCP.get_obs(j123))
		...
		@testset "getindex no-op collapsing n=$n, $f" for n in 1:3, f in (:, 1:N, "cell_id"=>Returns(true))
			job = counts_job
			for i in 1:n
				job = SCP.filter_obs(f, job)
			end
			@test isequal(forward!(SCP.get_matrix(job)), forward!(SCP.get_matrix(counts_job)))
			@test isequal(forward!(SCP.get_var(job)), forward!(SCP.get_var(counts_job)))
			@test isequal(forward!(SCP.get_obs(job)), forward!(SCP.get_obs(counts_job)))
		end
		...
		@testset "getindex perm/invperm" begin
			...
			jp = create_datamatrix_getindex_job(counts_job; obs_ind=perm)
			job = create_datamatrix_getindex_job(jp; obs_ind=iperm)
