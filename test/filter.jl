...
using SingleCellProjections: create_datamatrix_getindex_job
...
function run_filter_tests()
	@testset "Filtering" begin
		...
		var_annot_job = SingleCellProjections.table_getindex_job(Jobs.annotation(Jobs.get_var(counts_job), "name"), P:-4:1)

		obs_annot_df = select(fetch!(Jobs.get_obs(counts_job)), ["cell_id", "barcode"])[end:-5:1, :]
		obs_annot_job = SingleCellProjections.table_getindex_job(Jobs.annotation(Jobs.get_obs(counts_job), "barcode"), N:-5:1)
		...
		# @testset "filter $name" for (name,data_job) in (("counts",counts_job),)
			...
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; var_ind=1:2:P)
			...
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; obs_ind=1:2:N)
			@test forward!(f_job) == forward!(ref_job)
			...
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; var_ind=1:3:P, obs_ind=1:10:N)
			@test forward!(f_job) == forward!(ref_job)
			...
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; obs_ind=ref_obs_ind)
			@test forward!(f_job) == forward!(ref_job)
			...
			ref_obs_ind = findall(==("A"), data.obs.group)
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; var_ind=1:3:P, obs_ind=ref_obs_ind)
			@test forward!(f_job) == forward!(ref_job)
			# NB: Projection should fail unless the var IDs are identical!
			...
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; var_ind=ref_var_ind)
			@test forward!(f_job) == forward!(ref_job)
			...
			ref_var_ind = findall(>=("D"), data.var.name)
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; var_ind=ref_var_ind, obs_ind=1:10:N)
			@test forward!(f_job) == forward!(ref_job)
			# NB: Projection should fail unless the obs IDs are identical!
			...
			ref_var_ind = findall(>=("D"), data.var.name)
			ref_obs_ind = findall(==("A"), data.obs.group)
			ref_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; var_ind=ref_var_ind, obs_ind=ref_obs_ind)
			@test forward!(f_job) == forward!(ref_job)


			# annotations
			let var_mask = var_mask = in(var_annot_df.name).(data.var.name)
				f1_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; var_ind=findall(var_mask))
				f2_job = Jobs.filter_var(var_annot_df=>!ismissing, data_job)
				f3_job = Jobs.filter_var(var_annot_job=>!ismissing, data_job)
			...
			let obs_mask = obs_mask = in(obs_annot_df.barcode).(data.obs.barcode)
				f1_job = SingleCellProjections.create_datamatrix_getindex_job(data_job; obs_ind=findall(obs_mask))
				f2_job = Jobs.filter_obs(obs_annot_df=>!ismissing, data_job)
				f3_job = Jobs.filter_obs(obs_annot_job=>!ismissing, data_job)
