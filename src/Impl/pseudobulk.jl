...
function pseudobulk_table(action::Action, categories, basenames; do_project::Bool, kwargs...)
	...
	n_categories = fetched.(length_job.(categories))
	create_job(Preprocess(pseudobulk_table_impl), categories, basenames, n_categories; kwargs...)
end
pseudobulk_table_job(categories, basenames; kwargs...) =
	create_job(Projectable(pseudobulk_table), categories, basenames; kwargs...)
...
function pseudobulk_dm(::Mat, data, obs_cov_categories, obs_cov_ind, ::Any;
	...
	n_categories = length_job.(cov_categories)
	...
	if new_var_cov_categories !== nothing
		...
		mat = reshape_job(mat, :, n_obs_combinations)
	end
...
function pseudobulk_dm(::Var, data, args...;
	...
	n_new_var_combinations = prefetched(prod_job(length_job.(new_var_cov_categories)))
...
function pseudobulk_dm(::Obs, data, obs_cov_categories, ::Any, obs_cov_basenames; delim, obs_id_colname=nothing, kwargs...)
	pb_table_job = pseudobulk_table_job(obs_cov_categories, obs_cov_basenames; do_project=true)
...
function build_pseudobulk(::Preprocessing, data, args...; kwargs...)
	create_job(DataMatrixFunction(pseudobulk_dm), data, args...; kwargs...)
end
...
function pseudobulk(::Preprocessing, data, obs_covariate1, obs_covariates...; new_var_covariates=nothing, delim=nothing, kwargs...)
	...
	obs_cov_data = _extract_data_job.(Ref(obs), obs_cov_annots)
	obs_cov_basenames = fetched.(_extract_name.(obs_cov_annots))


	# obs_cov_categories = categories_job.(obs_cov_data, obs_cov_descs) # Something like this for TwoGroup support
	obs_cov_categories = categories_job.(obs_cov_data)
	obs_cov_ind = indexin_job.(obs_cov_data, obs_cov_categories)
	...
	if new_var_covariates !== nothing
		...
		new_var_cov_data = _extract_data_job.(Ref(obs), new_var_cov_annots)
		new_var_cov_basenames = fetched.(_extract_name.(new_var_cov_annots))

		new_var_cov_categories = categories_job.(new_var_cov_data)
		new_var_cov_ind = indexin_job.(new_var_cov_data, new_var_cov_categories)
	...
	create_job(Preprocess(build_pseudobulk), data, obs_cov_categories, obs_cov_ind, obs_cov_basenames; delim, kwargs..., pb_kwargs...)
end
...
function population_matrix_dm(::Var, args...;
                              ...
                              kwargs...)
	# Table for the new covariates, already repeated to match number of variables
	pb_table_job = pseudobulk_table_job(new_var_cov_categories, new_var_cov_basenames; do_project=false)
...
function build_population_matrix(::Preprocessing, args...; kwargs...)
	create_job(DataMatrixFunction(population_matrix_dm), args...; kwargs...)
end
...
function population_matrix(::Preprocessing, obs, obs_covariate1, obs_covariates...; new_var_covariates, delim=nothing, kwargs...)
	...
	delim = @something delim '_'
	create_job(Preprocess(build_population_matrix), obs_cov_categories, obs_cov_ind, obs_cov_basenames; delim, new_var_cov_categories, new_var_cov_ind, new_var_cov_basenames, kwargs...)
end
