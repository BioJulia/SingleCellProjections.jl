abstract type ProjectionModel2 end

function project2 end


# Just a fallback if we forget to define show for a model
Base.show(io::IO, ::MIME"text/plain", model::T) where T<:ProjectionModel2 =
	print(io, replace(string(nameof(T)),"Model"=>""))

function Base.show(io::IO, model::T) where T<:ProjectionModel2
