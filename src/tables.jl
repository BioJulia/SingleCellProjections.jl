...
# TODO: We will have a common pattern for many utility functions taking tables as input
...
get_colnames_pr(action, table) =
	create_spec(Preprocess(get_colnames_pre), forwarded_to_impl(action(table)))
...
# is_create_table_spec(x::Spec) = x.f in (create_table_pr, create_table_impl) # Should have been Projectable(create_table_pr), right?
